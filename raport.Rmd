...
htmltools::includeHTML("plots/plot1GC.html")
```
<br>
...
#Calculate distance needed for each place (50 competitors)
...
colnames(df_table_distance_and_place) <- c("Miejsce", "Dystans Normalizowany", "Lahti", "Ruka", "Wisła", "Zakopane")
```
```{r ch7}
...
![Wykres 14](plots/histogramWC_flying.jpg)
<br>

```{r ch8, echo=FALSE, message=FALSE, warning=FALSE}
#Read file
df <- read_csv(
  "data/cleanDataCompetitions.csv",
  col_names = TRUE,             
...
#Calculate distance needed for each place (50 competitors)
places <- quantile(df$distance_N, probs = seq(0, 1, length.out = 41))
places <- places[-c(1, length(places))]
vector <- 39:1
...
colnames(df_table_distance_and_place) <- c("Miejsce", "Dystans Normalizowany","Vikersund")
```
```{r ch9}
kable(df_table_distance_and_place)
```
